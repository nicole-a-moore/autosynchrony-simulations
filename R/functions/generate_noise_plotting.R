...
generate_noise <- function(beta, ## spectral exponent, between 0 (low autocorelation) and 1 (high autocorrelation)
  ...
  while(x <= n_ts) {
    ...
    ## if asynchronous, generate a new random element of phase each time
    if(p != 1) {
    ...
    ## remove mean and set standard deviation:
    noise_stable <- noise_stable*1/sqrt(var(noise_stable))*sqrt(sigma^2)
    noise_stable <- noise_stable - mean(noise_stable)
    #plot(x = 1:L1, y = noise_stable)
    noise_shifting <- noise_shifting*1/sqrt(var(noise_shifting))*sqrt(sigma^2)
    ...
    list_spectra[[x]] <- spectral
    
    x = x + 1
  }
  
  return(list(list_ts, list_colours, list_spectra))
}
