...
simulate_range_shifts <- function(p,
  ...
  r = 0.75 # maximum intrinsic rate of increase 
  K = 100 # mean carrying capacity 
  d = 0.1 # proportion of offspring dispersing
  icp = 0.1 ## intraspecific competition parameter
  L = 2000 # number of time steps to simulate
  ...
  path = "outputs/data-processed/range-shift-simulations_allee" # set path
  shift_rate = 0.2
  rep=1
  ...
  foreach(rep = 1:reps) %dopar% {
    ...
    while(t <= L) {
      ...
      if(t == 1) {
        ...
        lattice_N_it[1:(nrow/6),1:ncol,1] <- K/2 ## start with population size = carrying capacity / 2 in 1/6 of grid
        lattice_N_it[(nrow/6):nrow,1:ncol,1] <- 0 ## start with population size = 0 in other half 
        ...
        while(q <= (L-500)) {
          ...
          if(new_opt < nrow-1) {
            ...
            ## make growth rate plateau at -0.25
            lattice_E_it_array[1:nrow,1:ncol,q+500] = lattice_E_it_array[1:nrow,1:ncol,q+500] - 0.25
          }
          ## otherwise
          else {
            step = floor(shift_rate*q)
        ...
        filename =  paste0("outputs/data-processed/env-grids/range-shift-grid_allee_", rep, "_p", p, "_beta", beta, "_r", r, "_K", K, "_d", 
                           d, "_icp", icp, "_L", L, ".tif")
...
N[1] <- 5
for(t in 1:99){
  N[t+1] <- ricker_allee(N[t], r = 1.2, K = 100, A = 10)
}
plot(N, type = "l")
