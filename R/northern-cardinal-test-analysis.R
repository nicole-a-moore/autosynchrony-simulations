...
## using northern cardinal as an example
library(tidyverse)
library(tidyterra)
library(sf)
library(terra)
...
for(y in 1:length(unique(ebb_df$Year))) {
  
  year = unique(ebb_df$Year)[order(unique(ebb_df$Year))][y]
  
  if(year > 1980) {
    ## calculate 95th percentile of occupied cells
    cur <- ebb_df %>%
      filter(!TotalAbd == 0 & Year == year) %>%
      mutate(p95_w = modi::weighted.quantile(Y, w = TotalAbd, 0.95),
             p5_w = modi::weighted.quantile(Y, w = TotalAbd, 0.05)) %>%
      mutate(p95 = quantile(Y, 0.95), 
             p5 = quantile(Y, 0.05)) 
    
    p = cur %>%
      ggplot(aes(colour = TotalAbd)) +
      geom_sf(inherit.aes = FALSE, data = countries) +
      geom_sf(size = 0.5, data = filter(ebb_all, TotalAbd == 0 & Year == year), inherit.aes = FALSE, 
    ...
    ggsave(p, path = "outputs/figures/northern-cardinal", 
           filename = paste0("BBS_northern-cardinal_", year, ".png"), width = 4, height = 3)
  }
  else if (year == 1980){
    min_year = min(ebb_all$Year)
    
    ## calculate 95th percentile of occupied cells
    cur <- ebb_df %>%
      filter(!TotalAbd == 0 & Year == 1980) %>%
      mutate(p95_w = modi::weighted.quantile(Y, w = TotalAbd, 0.95),
             p5_w = modi::weighted.quantile(Y, w = TotalAbd, 0.05)) %>%
      mutate(p95 = quantile(Y, 0.95), 
             p5 = quantile(Y, 0.05)) 
    ...
    ggsave(p, path = "outputs/figures/northern-cardinal", 
           filename = paste0("BBS_northern-cardinal_", min_year, "to1980.png"), width = 4, height = 3)
  }
...
## play with some definitions of leading edge routes 
## 95th percentile of occupied cells 
for(y in 1:length(unique(ebb_df$Year))) {
  year = unique(ebb_df$Year)[order(unique(ebb_df$Year))][y]
  ...
  cur <- ebb_df %>%
    filter(!TotalAbd == 0 & Year == year) %>%
    mutate(p95_w = modi::weighted.quantile(Y, w = TotalAbd, 0.05)) %>%
    mutate(p95 = quantile(Y, 0.05)) %>%
    mutate(edge = ifelse(Y >= p95, NA, "yes"))
  
  p = cur %>%
    filter(edge == "yes") %>%
    ggplot(aes(colour = TotalAbd)) +
    geom_sf(inherit.aes = FALSE, data = countries) +
    geom_sf(size = 0.5, data = filter(ebb_df, TotalAbd == 0 & Year == year, Y <= unique(cur$p95)), inherit.aes = FALSE, 
            fill = "transparent", linewidth = 0.0001, shape = 1) +
    geom_sf(size = 0.5) +
    scale_colour_gradient(trans = "log", limits = c(1, 191)) +
    scale_x_continuous(limits = c(-130,-60)) +
    scale_y_continuous(limits = c(25,60)) +
    labs(colour = "Total abundance", title = year) +
    geom_hline(aes(yintercept = unique(cur$p95)), colour = "red") +
    geom_hline(aes(yintercept = unique(cur$p95_w)), colour = "blue")
  
  ggsave(p, path = "outputs/figures/american-goldfinch", 
         filename = paste0("trailing-edge_BBS_american-goldfinch_", year, ".png"), width = 4, height = 3)
}
